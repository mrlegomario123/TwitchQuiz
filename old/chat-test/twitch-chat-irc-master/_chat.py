...
class TwitchChatIRC():
    ...
    __DEFAULT_NICK = 'captainpovey'
    # __DEFAULT_PASS = 'oauth:qwynekuzj717pqvs6oimwj9p3i1l8u'
    __DEFAULT_PASS = "oauth:wc3aieab2y5ucaoosmyxu2zudywym6"
    __PORT = 6667
    ...
    def __init__(self, username=None, password=None):
        ...
        self.__NICK = 'captainpovey'
        self.__PASS = 'oauth:qwynekuzj717pqvs6oimwj9p3i1l8u'
    ...
    def __print_message(self, message):
        print('[' + message['tmi-sent-ts'] + ']', message['display-name'] + ':',
              emoji.demojize(message['message']).encode('utf-8').decode('utf-8', 'ignore'))
    ...
    def listen(self, channel_name, messages=[], timeout=None, message_timeout=1.0, on_message=None, buffer_size=4096,
               message_limit=None, output=None):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Send and receive Twitch chat messages over IRC with python web sockets. For more info, go to https://dev.twitch.tv/docs/irc/guide')

    parser.add_argument('channel_name', help='Twitch channel name (username)')
    parser.add_argument('-timeout', '-t', default=None, type=float,
                        help='time in seconds needed to close connection after not receiving any new data (default: None = no timeout)')
    parser.add_argument('-message_timeout', '-mt', default=1.0, type=float,
                        help='time in seconds between checks for new data (default: 1 second)')
    ...
    parser.add_argument('-message_limit', '-l', default=None, type=int,
                        help='maximum amount of messages to get (default: None = unlimited)')

    parser.add_argument('-username', '-u', default=None, help='username (default: None)')
    parser.add_argument('-oauth', '-password', '-p', default=None,
                        help='oath token (default: None). Get custom one from https://twitchapps.com/tmi/')
